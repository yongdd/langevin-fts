...
! if BOX_DIMENSION == 2, input parameters related to       y direction  will be ignored.
  integer, parameter :: BOX_DIMENSION = 3
! grid parameters
  type geomtry
...
!   grid number for A fraction
    ...
    y%lo = 0
    z%lo = 0
    if ( x%is_periodic) x%lo = 1
    if ( y%is_periodic) y%lo = 1
    if ( z%is_periodic) z%lo = 1
...
!   error_level measures the "relative distance" between the input and output fields
    ...
    error_level = sqrt(multidot(wout-w,wout-w)/(multidot(w,w)+1.0d0))

!   print iteration # and error levels
    write(*,'(I6,F8.3,2F13.9)') iter, mix, energy_tot, error_level
!   condition to start anderson mixing
    if(error_level < 5.0d-19) start_anderson = min(start_anderson,iter)
!   conditions to end the iteration
    if(error_level < 5.0d-19) exit
