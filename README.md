...
#### Efficient Computation of Partial Partition Functions and Polymer Concentrations
  ...
  3. A polymer branch is represented by a text code, which should be unique regardless of permutation of children branches and contain information of the children branches. Text code of a linear homopolymer is represented by concatenating monomer type and segment number (e.g. `A10`, `B20`, ...). If children branches are attached to one end of a homopolymer, their text codes are enumerated in parenthesis in ascending order and it is concatenated with text code of parent homopolymer. For instance, if three branches, `B5`, `A5` and `C5`, are attached to a homopolymer `A10`, its the text code become `(A5B5C5)A10`. Note that children branches must be enumerated in ascending order after sorting their text codes, so that it can be uniquely represented. If two branches, as an another example, `(A5B6)B6` and `(A3B6)A6`, are attached to one end of a `B4` homopolymer, its text code is `((A3B6)A6(A5B6)B6)B4`. Children branches are recursively enumerated in ascending order in the parenthesis. This text code can be readily obtained by using a recursive function. Note that this representation is for finding the same branch shape and its dependencies, not for expressing the entire polymer chain. For exercise, let us find all text codes in linear ABC triblock copolymer composed of `A4`, `B4`, and `C4` blocks. We need to find text codes for each block and each direction, so there are 6 text codes.
  ...
  4. A key of branch is defined as a text that rightmost segment number is truncated from its text code. The height of a branch can be determined by counting the number of open parenthesis of its key. For instance, the heights of `A`, `(A4A4)A`, and `((A4)B5)A` are 0, 1, and 2, respectively.
...
#### Scheduling for the Parallel Computations
  ...
  4. It is really bad idea to assign a CPU core to each scheduler and then run them independently. Because their computations will be not synchronized, it is not guarantee to satisfy the dependencies of branches. So, we need to synchronize the computations whenever computation of branches are finished or computation of new branches begin. Thus, entire timeline is split to smaller time spans based on the schedule planned above.

#### Reducing GPU Memory Usage
  Partial partition functions of all segments are stored in the GPU's global memory to minimize data transfer between main memory and global memory, because data transfer is expensive. However, this method limits the size of the grid number and segment number. If the GPU memory space is not enough to run simulations, the partial partition functions should be stored in main memory instead of GPU memory. To reduce data transfer time, `concurrent data copy and kernel execution` can be utilized, which simultaneously transfers data and executes kernels. An example applied to AB diblock copolymers is provided in the appendix of [*Macromolecules* **2021**, 54, 11304]. In the current implementation, the polymer concentrations are computed on the CPU, whereas the calculations were also parallelized on the GPU in the previous implementation. To enable this GPU memory saving option, set 'reduce_gpu_memory_usage' to 'True' in the example script. If this option is enabled, the factory will create an instance of CudaPseudoContinuousReduceMemory or CudaPseudoDiscreteReduceMemory.
