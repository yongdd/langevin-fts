...
#### Reducing GPU Memory Usage
  Partial partition functions of all segments are stored in the GPU's global memory and data transfer between main memory and global memory is minimized, because data transfer is expensive. However, this method limits the size of the grid number and segment number. If the GPU memory space is not enough to run the simulation, the the partial partition functions should be stored in main memory instead of GPU memory. A naive approach would be to pause computation on the GPU after computation of one segment is complete on the GPU, and transfer the computed partial partition function to main memory. A better approach would be to leverage `concurrent copy and kernel execution`, which transfers data and executes kernels simultaneously. As a result, data transfer costs can be hided. An example applied to AB diblock copolymers is provided in the appendix of [*Macromolecules* **2021**, 54, 11304]. In the current implementation, the polymer concentrations are calculated using the CPU, whereas in the previous implementation for AB diblock copolymers the calculations were also parallelized on the GPU. To enable this GPU memory saving option, set 'reduce_gpu_memory_usage' to 'True' in the example script. If this option is enabled, the factory will creates an instance of CudaPseudoContinuousReduceMemory or CudaPseudoDiscreteReduceMemory.
