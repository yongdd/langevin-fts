...
* Automatic Optimization of Partition Function Computation of Arbitrary Acyclic Branched Polymer Mixture
...
+ It should produce the same results within the machine precision regardless of platform (CUDA or MKL) and use of superposition. After changing "platform" and "use_superposition", run a few iterations with the same simulation parameters. And check if it outputs the same results.
...
#### Efficient Computation of Partial Partition Functions
  ...
  3. A polymer block and branches attached thereto are uniquely represented by a text key. The text keys of the attached branches are enumerated in a parenthesis.
      + Example 1: There are `A` homopolymers with segment numbers of 6. Suppose the two chain ends are at `s=0` and `s=6`. In forward direction computation of partial partition function, the computed chain length will increase from 0 to 6 (continuous chain), and the text key will change from `A0` to `A6`. In backward direction computation, the text key also will change from `A0` to `A6`.
      ...
      + Example 4: There are AB diblock copolymers with 4 `A` segments and 6 `B` segments. In forward direction computation, text keys are `A0`, `(A4)B6`, `A4` at chain ends and the junction. In back direction computation, text keys are `B0`, `(B6)A4`, `B6` at chain ends and the junction.
  4. In order to avoid redundant calculation of the partial partition functions of side chains and branches, a bottom-up `dynamic programming` approach is adopted. The partial partition functions of simplest side chains are first computed. Then, using these solutions, the partial partition functions of more complex branches are calculated. The last step is repeated up to the most complex branches.
      + Example 1: There is a mixture of short and long `A` homopolymers with segment numbers of 4 and 6, respectively. It is not necessary to separately compute the partial partition functions of shorter `A` homopolymers and complementary partition functions. Thus, only necessary computations are from *q*(**r**,`A0`) to *q*(**r**,`A6`). In the first step, enumerate all text keys for each polymer and for each direction, then we obtain [`A6`, `A6`, `A4`, `A4`] in descending order. In the second step, remove duplicated keys and leave only keys of the longest ones, then we only have [`A6`]. Now, we know that we only need to compute the partial partition functions from *q*(**r**,`A0`) to *q*(**r**,`A6`).
     + Example 2: There are 3-arm star `A` homopolymers. Each arm is composed of 4 `A` segments. It is only necessary to compute the partial partition functions of one arm. In the first step, enumerate all text keys for each block and for each direction, then we obtain [`A4`, `A4`, `A4`, `(A4A4)A4`, `(A4A4)A4`, `(A4A4)A4`] in descending order.  In the second step, remove duplicated keys and leave only keys of the longest ones, then we only have [`A4`, `(A4A4)A4`]. Now, we know that we only need to compute the partial partition functions from *q*(**r**,`A0`) to *q*(**r**,`A4`), and from *q*(**r**,`(A4A4)A0`) to *q*(**r**,`(A4A4)A4`). In actual implementation, keys are `A` and `(A4A4)A` and 4 and 4 are their corresponding values, respectively.
     + Example 3: There is a mixture of `A` homopolymers and symmetric ABA triblock copolymers. The `A` homopolymer is composed of 6 `A` segments. For the ABA triblock, 4 `A`, 5 `B` and 4 `A` segments are linearly attached. In the first step, enumerate all text keys for each block and for each direction, then we obtain [`A6`, `A6`, `A4`, `A4`,`(A4)B5`, `(A4)B5`, `((A4)B5)A4`, `((A4)B5)A4`] in descending order.  In the second step, remove duplicated keys and leave only keys of the longest one, then we only have [`A6`, `(A4)B5`, `((A4)B5)A4`]. Now, we know that we only need to compute the partial partition functions from *q*(**r**,`A0`) to *q*(**r**,`A6`), from *q*(**r**,`(A4)B0`) to *q*(**r**,`(A4)B5`), and from *q*(**r**,`(((A4)B5)A0`) to *q*(**r**,`((A4)B5)A4`).
  5. When 'use_superposition' is set to 'True', the partial partition functions of side chains are efficiently computed by exploiting the superposition principle, since the modified diffusion equation is a linear equation. This trick was first introduced in [*Macromolecules* **2019**, 52, 1794] for bottle-brushes (see appendix) and it is generalized in this library (implementation detail will be added). My implementation is sub optimal for some cases.

#### Scheduling for the Parallel Computations
  1. If one partial partition function is independent of the other partial partition function. They can be computed in parallel. For instance, the partial partition function and the complementary partial partition function of AB diblock copolymers can be computed in parallel. However, scheduling partial partition function computations for a mixture of arbitrary acyclic branched polymers is a complex problem.
  2. The height of a branch can be determined by counting the number of open parenthesis of its text key. For instance, the heights of `A`, `(A4A4)A4`, and `((A4)B5)A4` are 0, 1, and 2, respectively. A naive parallel implementation is to compute partial partition functions of the same height in parallel. For instance, `A6` and `B4` can be computed in parallel for AB diblock copolymer with 6`A` segments and 4`B`. After computation of 0 height is done, `(A6)B4` and `(B4)A6` with height of 1 can be computed in parallel. However, the optimal parallel schedule follows. First, compute `A4` and `B4` in parallel. Second, compute `A6` and `(B4)A2` in parallel. Next, compute `(A6)B4` and `(B4)A6` in parallel.
  3. For parallel computation scheduling, this library uses a simple `greedy algorithm`. First, select keys with height of 0, and put them one by one into a stream scheduler (one of the CPUs for CPU version or one of the cuFFT batches for CUDA version) whose reserved total computation time is the smallest. Next, select keys with height of 1, find when dependencies of each key will be resolved, sort them with the resolved time in ascending order, and put them one by one into a stream scheduler whose reserved total computation time is the smallest. Repeat this process after increasing height by 1, until every keys are scheduled. This approach is heuristic and is not guaranteed to minimize the makespan. As far as I know, scheduling tasks to minimize the makespan is an NP-hard problem.
  ...
  5. The CPU version uses up to 8 and 4 CPUs for the continuous and discrete chain models, respectively. The CUDA version uses batched cuFFT with a maximum batch size of 2.
