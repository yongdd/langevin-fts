...
* Automatic Optimization of Partial Partition Function Computation (**beta**)
...
#### Scheduling for the Parallel Computations
  ...
  2. The height of a branch can be determined by counting the number of open parenthesis of its text key. For instance, the heights of `A`, `(A4A4)A4`, and `((A4)B5)A4` are 0, 1, and 2, respectively. A naive implementation is to compute partial partition functions of the same height in parallel. For instance, `A6` and `B4` can be computed in parallel for AB diblock copolymer with 6`A` segments and 4`B`. After computations of height of 0 is done, `(A6)B4` and `(B4)A6` whose heights are 1 can be computed in parallel. However, the optimal parallel schedule follows. First, compute `A4` and `B4` in parallel. Next, compute `A6` and `(B4)A2` in parallel. Finally, compute `(A6)B4` and `(B4)A6` in parallel.
