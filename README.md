...
#### Efficient Computation of Partial Partition Functions and Polymer Concentrations
  ...
  3. A polymer branch is uniquely represented by a text code. Text codes of the children branches are enumerated in a parenthesis of the text code of a parent branch in ascending order.
      ...
      + Example 2: Two branches, `B6` and `A3`, are attached to one end of a `A6` homopolymer. At the other end of `A6` homopolymer, the entire chain is expressed as `(A3B6)A6`. Children branches should be enumerated in ascending order in the parenthesis.
      + Example 3: Two branches, `(A5B6)B6` and `(A3B6)A6`, are attached to one end of a `B4` homopolymer. At the other end of `B4` homopolymer, the entire chain is expressed as `((A3B6)A6(A5B6)B6)B4`. Children branches are also enumerated in ascending order in the parenthesis.
  ...
  4. In order to avoid recomputation of the partial partition functions of the same branch shape and to achieve minimal propagator iterations, a bottom-up `dynamic programming` approach is adopted. The partial partition functions of simplest branches are first computed. Then, using these solutions, the partial partition functions of more complex branches are calculated. The last step is repeated up to the most complex branches.
      + Example 1: There is a mixture of short and long `A` homopolymers with segment numbers of 4 and 6, respectively. It is not necessary to separately compute the partial partition functions of shorter `A` homopolymers and complementary partition functions. Thus, only necessary computations are from *q*(**r**,`A0`) to *q*(**r**,`A6`). Let us find the only necessary computations using text codes. In the first step, enumerate all text codes for each polymer and for each direction, then we obtain [`A6`, `A6`, `A4`, `A4`] in descending order. In the second step, remove duplicated codes and leave only codes of the longest ones, then we only have [`A6`]. Now, we have found that we only need to compute the partial partition functions from *q*(**r**,`A0`) to *q*(**r**,`A6`).
      + Example 2: There are 3-arm star `A` homopolymers. Each arm is composed of 4 `A` segments. It is only necessary to compute the partial partition functions of one arm. Let us find the only necessary computations using text codes. In the first step, enumerate all text codes for each block and for each direction, then we obtain [`A4`, `A4`, `A4`, `(A4A4)A4`, `(A4A4)A4`, `(A4A4)A4`] in descending order.  In the second step, remove duplicated codes and leave only codes of the longest ones, then we only have [`A4`, `(A4A4)A4`]. Now, we have found that we only need to compute the partial partition functions from *q*(**r**,`A0`) to *q*(**r**,`A4`), and from *q*(**r**,`(A4A4)A0`) to *q*(**r**,`(A4A4)A4`). In actual implementation, `A` and `(A4A4)A` are used as keys, and 4 and 4 are their corresponding values, respectively.
      + Example 3: There is a mixture of `A` homopolymers and symmetric ABA triblock copolymers. The `A` homopolymer is composed of 6 `A` segments. For the ABA triblock, 4 `A`, 5 `B` and 4 `A` segments are linearly connected. Let us find the only necessary computations using text codes. In the first step, enumerate all text codes for each block and for each direction, then we obtain [`A6`, `A6`, `A4`, `A4`,`(A4)B5`, `(A4)B5`, `((A4)B5)A4`, `((A4)B5)A4`] in descending order.  In the second step, remove duplicated codes and leave only codes of the longest one, then we only have [`A6`, `(A4)B5`, `((A4)B5)A4`]. Now, we have found that we only need to compute the partial partition functions from *q*(**r**,`A0`) to *q*(**r**,`A6`), from *q*(**r**,`(A4)B0`) to *q*(**r**,`(A4)B5`), and from *q*(**r**,`(((A4)B5)A0`) to *q*(**r**,`((A4)B5)A4`).
  5. If we want to obtain all partial partition functions, above approach is optimal. In the most cases, however, we want to obtain only full partition functions and polymer concentrations. In these cases, the polymer concentrations of side chains are efficiently computed by exploiting the superposition principle, since the modified diffusion equation is a linear equation. This trick was first introduced in [*Macromolecules* **2019**, 52, 1794] for bottlebrushes (see appendix) and it is generalized in this library (implementation detail will be added). When this trick is utilized, we cannot trace block-wise concentrations and individual partial partition functions. Thus, 'pseudo.get_polymer_concentration()' and 'pseudo.get_partial_partition()' are disabled, if 'use_superposition' is set to 'False'. In this library, the polymer concentrations are obtained with minimal propagator iterations for the following cases. In other cases, it is at least sub optimal.
...
#### Scheduling for the Parallel Computations
  ...
  2. The height of a branch can be determined by counting the number of open parenthesis of its key. (A key of branch is defined as a text that segment number of right side is truncated from its text code) For instance, the heights of `A`, `(A4A4)A`, and `((A4)B5)A` are 0, 1, and 2, respectively. A naive implementation is to compute partial partition functions of the same height in parallel. For instance, let us consider AB diblock copolymer with 6 `A` segments and 4 `B`. `A` and `B`, whose heights are 0, can be computed in parallel up to `A6` and `B4`, respectively. After computations of height of 0 is done, `(A6)B` and `(B4)A`, whose heights are 1, can be computed in parallel up to `(A6)B4` and `(B4)A6`, respectively. However, this plan is not optimal for this asymmetric AB diblock copolymers. Because `(B4)A` have to wait until `A6` is finished, but `(B4)A` can be computed as soon as `B4` is finished. The optimal parallel plan follows. First, compute `A` and `B` up to `A4` and `B4` in parallel. Next, continue the computations of `A` and `(B4)A` up to `A6` and `(B4)A2` in parallel. Finally, compute `(A6)B` and `(B4)A` in parallel until `(A6)B4` and `(B4)A6`.
  3. For parallel computation scheduling, this library uses a simple `greedy algorithm`. First, prepare a scheduler for each CPU core. (Following explanation is based on CPU version, but the same algorithm can be utilized for scheduling cuFFT batches in the CUDA version) They are initially empty. Second, select keys with height of 0, and put them one by one into the scheduler whose total reserved computation time is the smallest. Next, select keys with height of 1, find the time that each key is ready by resolving its dependencies, sort them with the resolved time in ascending order, and put them one by one into the scheduler whose total reserved computation time is the smallest. Repeat this process until all keys are scheduled. Increase the height by 1 for each iteration. 
  4. Whenever computations are completed or new computations begin, it is necessary to reassign jobs to CPU cores. Thus entire timeline is split to smaller time spans based on the schedule planned above. As a result, the number of active CPU cores can be varying during the computations based on polymer architectures and the dependencies of branches. For the AB diblock copolymer, always two CPU cores are active. However, this approach is heuristic and is not guaranteed to minimize the makespan. As far as I understand, scheduling tasks to minimize the makespan for an arbitrary polymer architecture is an NP-hard problem. 
  5. The CPU version uses up to 4 CPUs, and the CUDA version uses batched cuFFT with a maximum batch size of 2.

#### Reducing GPU Memory Usage
  For computational efficiency, partial partition functions of all contour steps are stored in the global memory of GPU. Because the cost of data transfer between main and global memories is expensive. However, this approach limits the size of grid number and segment number. If GPU memory space is insufficient to run a simulation, we need to store the partial partition functions in the main memory instead of GPU memory. A naive approach is to transfer computed partition function into the main memory as soon as computation of one contour step on GPU is finished, after stopping the computation. The better approach is utilizing `device overlapping`, which transfers data and executes a kernel simultaneously. As a result, we can hide the cost of data transfer. The detailed explanation for AB diblock copolymer is provided in the appendix of [*Macromolecules* **2021**, 54, 11304]. In the current implementation, polymer concentrations are computed using CPU, whereas their computations were also parallelized on GPU in the previous implementation for AB diblock copolymer. To enable this gpu memory saving option, set 'reduce_gpu_memory_usage' to 'True' in the example scripts. Then a factory will create an instance of CudaPseudoContinuousReduceMemory or CudaPseudoDiscreteReduceMemory. 
