...
#### Scheduling for the Parallel Computations
  ...
  3. For parallel computation scheduling, this library uses a simple `greedy algorithm`. First, select keys with height of 0, and put them one by one into a stream scheduler (one of the CPUs for CPU version or one of the cuFFT batches for CUDA version) whose reserved total computation time is the shortest. Next, select keys with height of 1, find when dependencies of each key will be resolved, and sort them with the resolved time in ascending order, and put them one by one into a stream scheduler whose reserved total computation time is the shortest. Repeat this process after increasing height by 1 until every keys are scheduled. This approach is heuristic and is not guaranteed to minimize the makespan. As far as I know, scheduling tasks to minimize the makespan is an NP-hard problem.
