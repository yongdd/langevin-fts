...
class DeepLFTS:
    def __init__(self, data_shape, train_model, model_file = None):
    ...
    def train_model(self, folder_name, out_name) :
        ...
        X_train = np.zeros([n_train, 1, mx, my, mz])
        X_test  = np.zeros([n_test,  1, mx, my, mz])
        ...
        train_loader = DataLoader(tds, batch_size=self.batch_size, shuffle=True)
    ...
    def predict(self, w_minus) :
        ...
        X_test[0,0,:,:,:] = w_minus/self.scale
        with torch.no_grad():
            w_plus = self.net(torch.Tensor(X_test).to(self.device)).cpu().detach().numpy()*self.scale
            w_plus = w_plus.reshape(self.mx, self.my, self.mz)
            return w_plus

def save_training_data(w_minus, w_plus, file_name) :
    np.save(file_name,np.stack((w_minus,w_plus)))
