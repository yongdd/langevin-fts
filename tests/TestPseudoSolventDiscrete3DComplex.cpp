...
int main()
{
    try
    {
        ...
        // double w_a_real[M] = {0.183471406e+0,0.623968915e+0,0.731257661e+0,0.997228140e+0,0.961913696e+0,
        ...
        //                 };
        // double w_b_real[M] = {0.113822903e-1,0.330673934e+0,0.270138412e+0,0.669606774e+0,0.885344778e-1,
        ...
        T phi_a[M]={0.0}, phi_solvent[M]={0.0};
        T w_a[M]={0.0}, w_b[M]={0.0};
        ...
        for(int i=0; i<M; i++)
        {
            // w_a[i] = T{w_a_real[i], 0.0};
            // w_b[i] = T{w_b_real[i], 0.0};
            // w_a[i] = T{dist(rng), 0.0};
            // w_b[i] = T{dist(rng), 0.0};
            // w_a[i] = T{0.0, 0.0};
            // w_b[i] = T{0.0, 0.0};
            // w_a[i] = T{0.0, dist(rng)};
            // w_b[i] = T{0.0, dist(rng)};
            w_a[i] = T{dist(rng), dist(rng)};
            w_b[i] = T{dist(rng), dist(rng)};
            // std::cout << w_a[i] << ", " << w_b[i] << std::endl;
        }

        Molecules* molecules_1 = new Molecules("Discrete", 1.0/NN, bond_lengths);
        molecules_1->add_polymer(0.6, blocks_1, {});
        molecules_1->add_polymer(0.4, blocks_2, {});

        Molecules* molecules_2 = new Molecules("Discrete", 1.0/NN, bond_lengths);
        molecules_2->add_polymer(0.6, blocks_1, {});
        molecules_2->add_solvent(0.4, "A");

        PropagatorComputationOptimizer* propagator_computation_optimizer_1 = new PropagatorComputationOptimizer(molecules_1, false);
        propagator_computation_optimizer_1->display_blocks();
        propagator_computation_optimizer_1->display_propagators();

        PropagatorComputationOptimizer* propagator_computation_optimizer_2 = new PropagatorComputationOptimizer(molecules_2, false);
        propagator_computation_optimizer_2->display_blocks();
        propagator_computation_optimizer_2->display_propagators();

        std::vector<PropagatorComputation<T>*> solver_1_list;
        ...
        #endif
        #ifdef USE_CPU_MKL
        solver_name.push_back("CpuComputationDiscrete MKL, Aggregation=false");
        solver_name.push_back("CpuComputationReduceMemoryDiscrete MKL, Aggregation=false");
        #endif
        #ifdef USE_CUDA
        ...
        cb_1_list.push_back(new CpuComputationBox<T>({II,JJ,KK}, {Lx,Ly,Lz}, {}));
        solver_1_list.push_back(new CpuComputationDiscrete<T>(cb_1_list.end()[-2], molecules_1, propagator_computation_optimizer_1));
        solver_1_list.push_back(new CpuComputationReduceMemoryDiscrete<T>(cb_1_list.end()[-1], molecules_1, propagator_computation_optimizer_1));
        #endif
        #ifdef USE_CPU_MKL
        cb_1_list.push_back(new CpuComputationBox<T>({II,JJ,KK}, {Lx,Ly,Lz}, {}));
        cb_1_list.push_back(new CpuComputationBox<T>({II,JJ,KK}, {Lx,Ly,Lz}, {}));
        solver_1_list.push_back(new CpuComputationDiscrete<T>(cb_1_list.end()[-2], molecules_1, propagator_computation_optimizer_1, FFTBackend::MKL));
        solver_1_list.push_back(new CpuComputationReduceMemoryDiscrete<T>(cb_1_list.end()[-1], molecules_1, propagator_computation_optimizer_1, FFTBackend::MKL));
        #endif
        #ifdef USE_CUDA
        cb_1_list.push_back(new CudaComputationBox<T>({II,JJ,KK}, {Lx,Ly,Lz}, {}));
        cb_1_list.push_back(new CudaComputationBox<T>({II,JJ,KK}, {Lx,Ly,Lz}, {}));
        solver_1_list.push_back(new CudaComputationDiscrete<T>(cb_1_list.end()[-2], molecules_1, propagator_computation_optimizer_1));
        solver_1_list.push_back(new CudaComputationReduceMemoryDiscrete<T>(cb_1_list.end()[-1], molecules_1, propagator_computation_optimizer_1));
        #endif

        #ifdef USE_CPU_FFTW
        cb_2_list.push_back(new CpuComputationBox<T>({II,JJ,KK}, {Lx,Ly,Lz}, {}));
        cb_2_list.push_back(new CpuComputationBox<T>({II,JJ,KK}, {Lx,Ly,Lz}, {}));
        solver_2_list.push_back(new CpuComputationDiscrete<T>(cb_2_list.end()[-2], molecules_2, propagator_computation_optimizer_2));
        solver_2_list.push_back(new CpuComputationReduceMemoryDiscrete<T>(cb_2_list.end()[-1], molecules_2, propagator_computation_optimizer_2));
        #endif
        #ifdef USE_CPU_MKL
        cb_2_list.push_back(new CpuComputationBox<T>({II,JJ,KK}, {Lx,Ly,Lz}, {}));
        cb_2_list.push_back(new CpuComputationBox<T>({II,JJ,KK}, {Lx,Ly,Lz}, {}));
        solver_2_list.push_back(new CpuComputationDiscrete<T>(cb_2_list.end()[-2], molecules_2, propagator_computation_optimizer_2, FFTBackend::MKL));
        solver_2_list.push_back(new CpuComputationReduceMemoryDiscrete<T>(cb_2_list.end()[-1], molecules_2, propagator_computation_optimizer_2, FFTBackend::MKL));
        #endif
        #ifdef USE_CUDA
        cb_2_list.push_back(new CudaComputationBox<T>({II,JJ,KK}, {Lx,Ly,Lz}, {}));
        cb_2_list.push_back(new CudaComputationBox<T>({II,JJ,KK}, {Lx,Ly,Lz}, {}));
        solver_2_list.push_back(new CudaComputationDiscrete<T>(cb_2_list.end()[-2], molecules_2, propagator_computation_optimizer_2));
        solver_2_list.push_back(new CudaComputationReduceMemoryDiscrete<T>(cb_2_list.end()[-1], molecules_2, propagator_computation_optimizer_2));
        #endif
        ...
        for(size_t n=0; n<solver_1_list.size(); n++)
        {
            ...
            std::cout<< "Check total_partition 1: "<< error << std::endl;
            if (solver_1_list[n]->check_total_partition() == false)
                return -1;

            std::cout<< "Check total_partition 2: "<< error << std::endl;
            if (solver_2_list[n]->check_total_partition() == false)
                return -1;
            
            T QQ_1 = solver_1_list[n]->get_total_partition(p);
            T QQ_2 = solver_2_list[n]->get_solvent_partition(s);
            error = std::abs((QQ_1-QQ_2)/(Lx*Ly*Lz));
            std::cout<< "Total Propagator error: "<< QQ_1 << ", "<< QQ_2 << ", " << error << std::endl;
            if (!std::isfinite(error) || error > 1e-7)
                return -1;

            for(int i=0; i<M; i++)
            {
                // std::cout<< "phi[i]: "<< phi_a[i] << ", "<< phi_solvent[i] << std::endl;
                diff_sq[i] = pow(std::abs(phi_a[i] - phi_solvent[i]),2);
            }
