...
int main()
{
    try
    {
        ...
        T phi_a[M]={0.0}, phi_b[M]={0.0};
        T w_a[M]={0.0}, w_b[M]={0.0};
        T q1_last[M], q2_last[M];
        ...
        for(int i=0; i<M; i++)
        {
            ...
            // std::cout << "w_a[i], w_b[i]: "<< w_a[i] << ", " << w_b[i] << std::endl;
        }

        Molecules* molecules = new Molecules("Discrete", 1.0/NN, bond_lengths);
        ...
        cb_list.push_back(new CudaComputationBox<T>({II,JJ,KK}, {Lx,Ly,Lz}, {}));
        solver_list.push_back(new CudaComputationDiscrete<T>(cb_list.end()[-2], molecules, propagator_computation_optimizer));
        solver_list.push_back(new CudaComputationReduceMemoryDiscrete<T>(cb_list.end()[-1], molecules, propagator_computation_optimizer));
        #endif
        ...
        for(size_t n=0; n<solver_list.size(); n++)
        {
            ...
            // solver->get_chain_propagator(q1_last, p, 0, 1, pc.get_block(0,1).n_segment);
            // for(int i=0; i<M; i++)
            //     std::cout<< "q1_last[i]: "<< q1_last[i] << std::endl;
            ...
            for(int i=0; i<M; i++)
                diff_sq[i] = pow(q2_last[i].real() - q2_last_ref[i],2);
            error = sqrt(*std::max_element(diff_sq.begin(),diff_sq.end()));
            std::cout<< "Complementary Propagator error: "<< error << std::endl;
            if (!std::isfinite(error) || error > 1e-7)
                return -1;

            T QQ = solver->get_total_partition(p);
            ...
            for(int i=0; i<M; i++)
                diff_sq[i] = pow(phi_a[i].real() - phi_a_ref[i],2);
            error = sqrt(*std::max_element(diff_sq.begin(),diff_sq.end()));
            ...
            for(int i=0; i<M; i++)
                diff_sq[i] = pow(phi_b[i].real() - phi_b_ref[i],2);
            error = sqrt(*std::max_element(diff_sq.begin(),diff_sq.end()));
