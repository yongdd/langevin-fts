#!/usr/bin/env python
"""Quick test to verify SDC fix on CUDA - 3D test."""
import os
...
def run_sdc_test(N_steps, chi_n=20.0, f=0.45, nx=32, lx=3.8):
    ...
    # Initialize with gyroid-like structure
    np.random.seed(42)
    x = np.linspace(0, lx, nx, endpoint=False)
    Y, X, Z = np.meshgrid(x, x, x)
    # Gyroid approximation
    gyroid = np.sin(2*np.pi*X/lx) * np.cos(2*np.pi*Y/lx) + \
             np.sin(2*np.pi*Y/lx) * np.cos(2*np.pi*Z/lx) + \
             np.sin(2*np.pi*Z/lx) * np.cos(2*np.pi*X/lx)
...
print("Testing SDC convergence on CUDA (3D gyroid)...")
print("=" * 50)
...
for N in N_values:
    ...
    print(f"N={N:4d}, ds={1.0/N:.4f}, H={H:.10f}")
...
for i in range(1, len(results)):
    ...
    N2, ds2, H2 = results[i]
    if abs(H2 - H1) > 1e-14:
        # Use Richardson extrapolation to estimate order
        if i < len(results) - 1:
