...
std::map<std::string, double>& PolymerChain::get_dict_bond_lengths()
{
...
struct polymer_chain_block& PolymerChain::get_block(int v, int u)
{
    if (edge_to_array.count(std::make_pair(v, u)) == 0)
        throw_with_line_number("There is no such edge (" + std::to_string(v) + ", " + std::to_string(u) + ").");
    return blocks[edge_to_array[std::make_pair(v, u)]];
}
std::vector<polymer_chain_block>& PolymerChain::get_blocks()
{
    return blocks;
}
...
// std::vector<std::string> PolymerChain::get_block_species()
// {
...
std::pair<std::string, int> PolymerChain::get_text_of_ordered_branches(int in_node, int out_node)
{
...
int PolymerChain::get_reduced_n_branches()
{
    return reduced_branches_max_segment.size();
}
std::vector<std::pair<std::string, int>> PolymerChain::key_to_deps(std::string key)
{
    std::stack<int> s;
...
std::string PolymerChain::key_to_species(std::string key){
    int key_start = 0;
...
std::map<std::string, int, std::greater<std::string>>& PolymerChain::get_reduced_branches_max_segment()
{
    return reduced_branches_max_segment;
}
int PolymerChain::get_reduced_branch_max_segment(std::string key)
{
