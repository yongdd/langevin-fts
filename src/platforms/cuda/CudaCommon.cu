...
//------------------------------------------------------------------------------
// Krogstad ETDRK4 kernels (Song et al. 2018)
//------------------------------------------------------------------------------

// Stage b (Eq. 7b): dst = a_hat + phi2_half * (N_a_hat - N_n_hat)
    ...
    const double* phi2_half, const int M)
{
    ...
    while (i < M)
    {
        cuDoubleComplex diff = cuCsub(N_a_hat[i], N_n_hat[i]);
        cuDoubleComplex scaled = make_cuDoubleComplex(phi2_half[i] * diff.x, phi2_half[i] * diff.y);
        dst[i] = cuCadd(a_hat[i], scaled);
        i += blockDim.x * gridDim.x;
    }
}

// Stage c (Eq. 7c): dst = E * q_hat + phi1 * N_n_hat + 2 * phi2 * (N_b_hat - N_n_hat)
    ...
    const double* E, const double* phi1, const double* phi2, const int M)
{
    ...
    while (i < M)
    {
        cuDoubleComplex diff = cuCsub(N_b_hat[i], N_n_hat[i]);
        cuDoubleComplex term1 = make_cuDoubleComplex(E[i] * q_hat[i].x, E[i] * q_hat[i].y);
        cuDoubleComplex term2 = make_cuDoubleComplex(phi1[i] * N_n_hat[i].x, phi1[i] * N_n_hat[i].y);
        cuDoubleComplex term3 = make_cuDoubleComplex(2.0 * phi2[i] * diff.x, 2.0 * phi2[i] * diff.y);
        dst[i] = cuCadd(cuCadd(term1, term2), term3);
        i += blockDim.x * gridDim.x;
    }
}

// Final (Eq. 7d): dst = c_hat + (4*phi3 - phi2)*(N_n_hat + N_c_hat) + 2*phi2*N_a_hat - 4*phi3*(N_a_hat + N_b_hat)
    ...
    const double* phi2, const double* phi3, const int M)
{
    ...
    while (i < M)
    {
        double coeff_nc = 4.0 * phi3[i] - phi2[i];  // coefficient for (N_n + N_c)
        cuDoubleComplex sum_nc = cuCadd(N_n_hat[i], N_c_hat[i]);
        cuDoubleComplex sum_ab = cuCadd(N_a_hat[i], N_b_hat[i]);

        // c_hat + coeff_nc * (N_n + N_c) + 2*phi2*N_a - 4*phi3*(N_a + N_b)
        cuDoubleComplex term1 = make_cuDoubleComplex(coeff_nc * sum_nc.x, coeff_nc * sum_nc.y);
        cuDoubleComplex term2 = make_cuDoubleComplex(2.0 * phi2[i] * N_a_hat[i].x, 2.0 * phi2[i] * N_a_hat[i].y);
        cuDoubleComplex term3 = make_cuDoubleComplex(-4.0 * phi3[i] * sum_ab.x, -4.0 * phi3[i] * sum_ab.y);

        dst[i] = cuCadd(cuCadd(cuCadd(c_hat[i], term1), term2), term3);
        i += blockDim.x * gridDim.x;
    }
}

// Stage b (Eq. 7b) for real coefficients: dst = a_hat + phi2_half * (N_a_hat - N_n_hat)
    ...
    const double* phi2_half, const int M)
{
    ...
    while (i < M)
    {
        dst[i] = a_hat[i] + phi2_half[i] * (N_a_hat[i] - N_n_hat[i]);
        i += blockDim.x * gridDim.x;
    }
}

// Stage c (Eq. 7c) for real coefficients: dst = E * q_hat + phi1 * N_n_hat + 2 * phi2 * (N_b_hat - N_n_hat)
    ...
    const double* E, const double* phi1, const double* phi2, const int M)
{
    ...
    while (i < M)
    {
        dst[i] = E[i] * q_hat[i] + phi1[i] * N_n_hat[i] + 2.0 * phi2[i] * (N_b_hat[i] - N_n_hat[i]);
        i += blockDim.x * gridDim.x;
    }
}

// Final (Eq. 7d) for real coefficients: dst = c_hat + (4*phi3 - phi2)*(N_n + N_c) + 2*phi2*N_a - 4*phi3*(N_a + N_b)
    ...
    const double* phi2, const double* phi3, const int M)
{
    ...
    while (i < M)
    {
        double coeff_nc = 4.0 * phi3[i] - phi2[i];
        double sum_nc = N_n_hat[i] + N_c_hat[i];
        double sum_ab = N_a_hat[i] + N_b_hat[i];
        dst[i] = c_hat[i] + coeff_nc * sum_nc + 2.0 * phi2[i] * N_a_hat[i] - 4.0 * phi3[i] * sum_ab;
        i += blockDim.x * gridDim.x;
    }
}
