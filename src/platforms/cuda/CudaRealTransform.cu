...
static int getFFTSize(CudaTransformType type, int N)
{
    switch (type) {
        case CUDA_DCT_1:
            return N - 1;  // DCT-1 uses (N-1)-size FFT for (N)-size input
        case CUDA_DCT_2:
        case CUDA_DCT_3:
            return N;
        case CUDA_DCT_4:
        case CUDA_DST_4:
            return 2 * N;
        case CUDA_DST_1:
            return N + 1;  // DST-1 uses (N+1)-size FFT for (N)-size input
        case CUDA_DST_2:
        case CUDA_DST_3:
            return 4 * N;
        default:
...
__global__ void kernel_dst1_preOp(
    const double* __restrict__ in,
    ...
    int N)
{
    ...
    if (itx < N) {
        work[itx + 1] = in[itx];
        work[fftSize - itx - 1] = -in[itx];
    }
}

__global__ void kernel_dst1_postOp(
    const cufftDoubleComplex* __restrict__ fft_result,
    double* __restrict__ out,
    int N)
{
    int itx = threadIdx.x;
    ...
    if (itx < N) {
        out[itx] = -fft_result[itx + 1].y;
    }
}

__global__ void kernel_dst2_preOp(
    const double* __restrict__ in,
    double* __restrict__ work,
    int N)
{
    ...
    if (itx < N) {
        double val = in[itx];
        work[2 * itx + 1] = val;
        work[fftSize - 2 * itx - 1] = -val;
    }
}

__global__ void kernel_dst2_postOp(
    const cufftDoubleComplex* __restrict__ fft_result,
    double* __restrict__ out,
    int N)
{
...
__global__ void kernel_dst3_preOp(
    const double* __restrict__ in,
    double* __restrict__ work,
    int N)
{
...
__global__ void kernel_dst3_postOp(
    const cufftDoubleComplex* __restrict__ fft_result,
    double* __restrict__ out,
    int N)
{
    ...
    if (itx < N) {
        out[itx] = -0.5 * fft_result[2 * itx + 1].y;
    }
}
    ...
    int Nx, int Ny)
{
    ...
    if (ix < Nx && iy < fftSize) {
        work[ix * fftSize + iy] = 0.0;
    }
    __syncthreads();

    if (ix < Nx && iy < Ny) {
        double val = input[ix * Ny + iy];
        work[ix * fftSize + 2 * iy + 1] = val;
        work[ix * fftSize + fftSize - 2 * iy - 1] = -val;
    }
}

__global__ void kernel_dst2_2d_postOp_y(
    const cufftDoubleComplex* fft_result,
    double* output,
    int Nx, int Ny)
{
    int ix = blockIdx.x;
    ...
    if (ix < Nx && iy < Ny) {
        output[ix * Ny + iy] = -fft_result[ix * fftSize + iy + 1].y;
    }
}
    ...
    int Nx, int Ny)
{
    ...
    if (iy < Ny && ix < fftSize) {
        work[iy * fftSize + ix] = 0.0;
    }
    __syncthreads();

    if (iy < Ny && ix < Nx) {
        double val = input[ix * Ny + iy];
        work[iy * fftSize + 2 * ix + 1] = val;
        work[iy * fftSize + fftSize - 2 * ix - 1] = -val;
    }
}
    ...
    int Nx, int Ny)
{
    ...
    if (iy < Ny && ix < Nx) {
        output[ix * Ny + iy] = -fft_result[iy * fftSize + ix + 1].y;
    }
}
    ...
    int Nx, int Ny)
{
    ...
    if (ix < Nx && iy < Ny) {
        double val = input[ix * Ny + iy];
        work[ix * fftSize + iy + 1] = val;
        work[ix * fftSize + 2 * Ny - iy - 1] = val;
        work[ix * fftSize + 2 * Ny + iy + 1] = -val;
        work[ix * fftSize + 4 * Ny - iy - 1] = -val;
    }
}
    ...
    int Nx, int Ny)
{
    ...
    if (ix < Nx && iy < Ny) {
        output[ix * Ny + iy] = -0.5 * fft_result[ix * fftSize + 2 * iy + 1].y;
    }
}
    ...
    int Nx, int Ny)
{
    ...
    if (iy < Ny && ix < Nx) {
        double val = input[ix * Ny + iy];
        work[iy * fftSize + ix + 1] = val;
        work[iy * fftSize + 2 * Nx - ix - 1] = val;
        work[iy * fftSize + 2 * Nx + ix + 1] = -val;
        work[iy * fftSize + 4 * Nx - ix - 1] = -val;
    }
}
    ...
    int Nx, int Ny)
{
    ...
    if (iy < Ny && ix < Nx) {
        output[ix * Ny + iy] = -0.5 * fft_result[iy * fftSize + 2 * ix + 1].y;
    }
}
    ...
    int Nx, int Ny, int Nz)
{
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    int fftSize = 4 * Nz;
    int total = Nx * Ny * fftSize;

    if (idx < total) {
        int iz = idx % fftSize;
        int iy = (idx / fftSize) % Ny;
        int ix = idx / (fftSize * Ny);

        int work_base = (ix * Ny + iy) * fftSize;
        work[work_base + iz] = 0.0;
    }
    ...
    if (idx2 < total2) {
        ...
        double val = in[(ix * Ny + iy) * Nz + iz];
        int work_base = (ix * Ny + iy) * fftSize;
        work[work_base + 2 * iz + 1] = val;
        work[work_base + fftSize - 2 * iz - 1] = -val;
    }
}
    ...
    int Nx, int Ny, int Nz)
{
    ...
    if (idx < total) {
        ...
        out[idx] = -fft_result[(ix * Ny + iy) * fftSize + iz + 1].y;
    }
}
    ...
    int Nx, int Ny, int Nz)
{
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    int fftSize = 4 * Ny;
    int total = Nx * Nz * fftSize;

    if (idx < total) {
        int iy = idx % fftSize;
        int iz = (idx / fftSize) % Nz;
        int ix = idx / (fftSize * Nz);

        int work_base = (ix * Nz + iz) * fftSize;
        work[work_base + iy] = 0.0;
    }
    ...
    if (idx2 < total2) {
        ...
        double val = in[(ix * Ny + iy) * Nz + iz];
        int work_base = (ix * Nz + iz) * fftSize;
        work[work_base + 2 * iy + 1] = val;
        work[work_base + fftSize - 2 * iy - 1] = -val;
    }
}
    ...
    int Nx, int Ny, int Nz)
{
    ...
    if (idx < total) {
        ...
        out[idx] = -fft_result[(ix * Nz + iz) * fftSize + iy + 1].y;
    }
}
    ...
    int Nx, int Ny, int Nz)
{
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    int fftSize = 4 * Nx;
    int total = Ny * Nz * fftSize;

    if (idx < total) {
        int ix = idx % fftSize;
        int iz = (idx / fftSize) % Nz;
        int iy = idx / (fftSize * Nz);

        int work_base = (iy * Nz + iz) * fftSize;
        work[work_base + ix] = 0.0;
    }
    ...
    if (idx2 < total2) {
        ...
        double val = in[(ix * Ny + iy) * Nz + iz];
        int work_base = (iy * Nz + iz) * fftSize;
        work[work_base + 2 * ix + 1] = val;
        work[work_base + fftSize - 2 * ix - 1] = -val;
    }
}
    ...
    int Nx, int Ny, int Nz)
{
    ...
    if (idx < total) {
        ...
        out[idx] = -fft_result[(iy * Nz + iz) * fftSize + ix + 1].y;
    }
}
    ...
    int Nx, int Ny, int Nz)
{
    ...
    if (idx2 < total2) {
        ...
        double val = in[(ix * Ny + iy) * Nz + iz];
        int work_base = (ix * Ny + iy) * fftSize;
        work[work_base + iz + 1] = val;
        work[work_base + 2 * Nz - iz - 1] = val;
        work[work_base + 2 * Nz + iz + 1] = -val;
        work[work_base + 4 * Nz - iz - 1] = -val;
    }
}
    ...
    int Nx, int Ny, int Nz)
{
    ...
    if (idx < total) {
        ...
        out[idx] = -0.5 * fft_result[(ix * Ny + iy) * fftSize + 2 * iz + 1].y;
    }
}
    ...
    int Nx, int Ny, int Nz)
{
    ...
    if (idx2 < total2) {
        ...
        double val = in[(ix * Ny + iy) * Nz + iz];
        int work_base = (ix * Nz + iz) * fftSize;
        work[work_base + iy + 1] = val;
        work[work_base + 2 * Ny - iy - 1] = val;
        work[work_base + 2 * Ny + iy + 1] = -val;
        work[work_base + 4 * Ny - iy - 1] = -val;
    }
}
    ...
    int Nx, int Ny, int Nz)
{
    ...
    if (idx < total) {
        ...
        out[idx] = -0.5 * fft_result[(ix * Nz + iz) * fftSize + 2 * iy + 1].y;
    }
}
    ...
    int Nx, int Ny, int Nz)
{
    ...
    if (idx2 < total2) {
        ...
        double val = in[(ix * Ny + iy) * Nz + iz];
        int work_base = (iy * Nz + iz) * fftSize;
        work[work_base + ix + 1] = val;
        work[work_base + 2 * Nx - ix - 1] = val;
        work[work_base + 2 * Nx + ix + 1] = -val;
        work[work_base + 4 * Nx - ix - 1] = -val;
    }
}
    ...
    int Nx, int Ny, int Nz)
{
    ...
    if (idx < total) {
        ...
        out[idx] = -0.5 * fft_result[(iy * Nz + iz) * fftSize + 2 * ix + 1].y;
    }
}
    ...
    : N_(N), type_(type), initialized_(false), d_work_(nullptr), d_x1_(nullptr)
{
    ...
    if (z2z) {
        ...
        if (cufftPlan1d(&plan_, N_ - 1, CUFFT_D2Z, 1) != CUFFT_SUCCESS) {
        ...
        if (cufftPlan1d(&plan_, 2 * (N_ + 1), CUFFT_D2Z, 1) != CUFFT_SUCCESS) {
            throw std::runtime_error("Failed to create cuFFT D2Z plan for DST-1");
        }
    } else {
        // DST-2, DST-3
...
void CudaRealTransform1D::execute(double* d_data)
{
    ...
    switch (type_) {
        case CUDA_DCT_1: {
            kernel_dct1_preOp<<<1, nThread, sizeof(double) * nThread>>>(d_data, d_x1_, N_ - 1, nThread);
            cufftExecD2Z(plan_, d_data, (cufftDoubleComplex*)d_work_);
            kernel_dct1_postOp<<<1, nThread>>>(d_work_, d_x1_, N_ - 1);
        ...
        case CUDA_DST_1: {
            kernel_dst1_preOp<<<1, nThread>>>(d_data, d_work_, N_);
            cufftExecD2Z(plan_, d_work_, (cufftDoubleComplex*)d_x1_);
            kernel_dst1_postOp<<<1, nThread>>>((cufftDoubleComplex*)d_x1_, d_data, N_);
            break;
        }
        case CUDA_DST_2: {
            kernel_dst2_preOp<<<1, nThread>>>(d_data, d_work_, N_);
            cufftExecD2Z(plan_, d_work_, (cufftDoubleComplex*)d_x1_);
            kernel_dst2_postOp<<<1, nThread>>>((cufftDoubleComplex*)d_x1_, d_data, N_);
            break;
        }
        case CUDA_DST_3: {
            kernel_dst3_preOp<<<1, nThread>>>(d_data, d_work_, N_);
            cufftExecD2Z(plan_, d_work_, (cufftDoubleComplex*)d_x1_);
            kernel_dst3_postOp<<<1, nThread>>>((cufftDoubleComplex*)d_x1_, d_data, N_);
            break;
        }
        case CUDA_DST_4: {
            int N2 = 2 * N_;
...
{
    if (type_ == CUDA_DCT_1) return N_ + 1;
    if (type_ == CUDA_DST_1) return N_ - 1;
    return N_;
}
...
void CudaRealTransform2D::executeY_DST2(double* d_data)
{
    ...
    cudaMemset(d_work_, 0, sizeof(double) * Nx_ * fftSize);
    kernel_dst2_2d_preOp_y<<<Nx_, fftSize>>>(d_data, d_work_, Nx_, Ny_);

    cufftDoubleComplex* d_freq = (cufftDoubleComplex*)d_x1_;
    cufftExecD2Z(plan_y_, d_work_, d_freq);

    kernel_dst2_2d_postOp_y<<<Nx_, Ny_>>>(d_freq, d_temp_, Nx_, Ny_);
}
...
void CudaRealTransform2D::executeY_DST3(double* d_data)
{
    ...
    cudaMemset(d_work_, 0, sizeof(double) * Nx_ * fftSize);
    kernel_dst3_2d_preOp_y<<<Nx_, fftSize>>>(d_data, d_work_, Nx_, Ny_);

    cufftDoubleComplex* d_freq = (cufftDoubleComplex*)d_x1_;
    cufftExecD2Z(plan_y_, d_work_, d_freq);

    kernel_dst3_2d_postOp_y<<<Nx_, Ny_>>>(d_freq, d_temp_, Nx_, Ny_);
}
...
void CudaRealTransform2D::executeX_DST2(double* d_data)
{
    ...
    cudaMemset(d_work_, 0, sizeof(double) * Ny_ * fftSize);
    kernel_dst2_2d_preOp_x<<<Ny_, fftSize>>>(d_temp_, d_work_, Nx_, Ny_);

    cufftDoubleComplex* d_freq = (cufftDoubleComplex*)d_x1_;
    cufftExecD2Z(plan_x_, d_work_, d_freq);

    kernel_dst2_2d_postOp_x<<<Ny_, Nx_>>>(d_freq, d_data, Nx_, Ny_);
}
...
void CudaRealTransform2D::executeX_DST3(double* d_data)
{
    ...
    cudaMemset(d_work_, 0, sizeof(double) * Ny_ * fftSize);
    kernel_dst3_2d_preOp_x<<<Ny_, fftSize>>>(d_temp_, d_work_, Nx_, Ny_);

    cufftDoubleComplex* d_freq = (cufftDoubleComplex*)d_x1_;
    cufftExecD2Z(plan_x_, d_work_, d_freq);

    kernel_dst3_2d_postOp_x<<<Ny_, Nx_>>>(d_freq, d_data, Nx_, Ny_);
}
...
{
    // Type-1 cannot be mixed
    if (isType1(type_x_) || isType1(type_y_) || isType1(type_z_)) {
        throw std::runtime_error("Type-1 transforms (DCT-1/DST-1) are not supported for 3D mixed transforms");
    }
...
void CudaRealTransform3D::executeZ_DST2(double* d_data)
{
    int blockSize = 256;
    int fftSize = 4 * Nz_;
    int total_preOp = Nx_ * Ny_ * fftSize;
    int numBlocks_preOp = (total_preOp + blockSize - 1) / blockSize;
    int numBlocks_postOp = (M_ + blockSize - 1) / blockSize;

    cudaMemset(d_work_, 0, sizeof(double) * Nx_ * Ny_ * fftSize);
    kernel_dst2_3d_preOp_z<<<numBlocks_preOp, blockSize>>>(d_data, d_work_, Nx_, Ny_, Nz_);

    cufftDoubleComplex* d_freq = (cufftDoubleComplex*)d_x1_;
    cufftExecD2Z(plan_z_, d_work_, d_freq);

    kernel_dst2_3d_postOp_z<<<numBlocks_postOp, blockSize>>>(d_freq, d_temp_, Nx_, Ny_, Nz_);
}
...
void CudaRealTransform3D::executeZ_DST3(double* d_data)
{
    ...
    cudaMemset(d_work_, 0, sizeof(double) * Nx_ * Ny_ * fftSize);
    kernel_dst3_3d_preOp_z<<<numBlocks_preOp, blockSize>>>(d_data, d_work_, Nx_, Ny_, Nz_);

    cufftDoubleComplex* d_freq = (cufftDoubleComplex*)d_x1_;
    cufftExecD2Z(plan_z_, d_work_, d_freq);

    kernel_dst3_3d_postOp_z<<<numBlocks_postOp, blockSize>>>(d_freq, d_temp_, Nx_, Ny_, Nz_);
}
...
void CudaRealTransform3D::executeY_DST2(double* d_data)
{
    int blockSize = 256;
    int fftSize = 4 * Ny_;
    int total_preOp = Nx_ * Nz_ * fftSize;
    int numBlocks_preOp = (total_preOp + blockSize - 1) / blockSize;
    int numBlocks_postOp = (M_ + blockSize - 1) / blockSize;

    cudaMemset(d_work_, 0, sizeof(double) * Nx_ * Nz_ * fftSize);
    kernel_dst2_3d_preOp_y<<<numBlocks_preOp, blockSize>>>(d_temp_, d_work_, Nx_, Ny_, Nz_);

    cufftDoubleComplex* d_freq = (cufftDoubleComplex*)d_x1_;
    cufftExecD2Z(plan_y_, d_work_, d_freq);

    kernel_dst2_3d_postOp_y<<<numBlocks_postOp, blockSize>>>(d_freq, d_temp_, Nx_, Ny_, Nz_);
}
...
void CudaRealTransform3D::executeY_DST3(double* d_data)
{
    ...
    cudaMemset(d_work_, 0, sizeof(double) * Nx_ * Nz_ * fftSize);
    kernel_dst3_3d_preOp_y<<<numBlocks_preOp, blockSize>>>(d_temp_, d_work_, Nx_, Ny_, Nz_);

    cufftDoubleComplex* d_freq = (cufftDoubleComplex*)d_x1_;
    cufftExecD2Z(plan_y_, d_work_, d_freq);

    kernel_dst3_3d_postOp_y<<<numBlocks_postOp, blockSize>>>(d_freq, d_temp_, Nx_, Ny_, Nz_);
}
...
void CudaRealTransform3D::executeX_DST2(double* d_data)
{
    int blockSize = 256;
    int fftSize = 4 * Nx_;
    int total_preOp = Ny_ * Nz_ * fftSize;
    int numBlocks_preOp = (total_preOp + blockSize - 1) / blockSize;
    int numBlocks_postOp = (M_ + blockSize - 1) / blockSize;

    cudaMemset(d_work_, 0, sizeof(double) * Ny_ * Nz_ * fftSize);
    kernel_dst2_3d_preOp_x<<<numBlocks_preOp, blockSize>>>(d_temp_, d_work_, Nx_, Ny_, Nz_);

    cufftDoubleComplex* d_freq = (cufftDoubleComplex*)d_x1_;
    cufftExecD2Z(plan_x_, d_work_, d_freq);

    kernel_dst2_3d_postOp_x<<<numBlocks_postOp, blockSize>>>(d_freq, d_data, Nx_, Ny_, Nz_);
}
...
void CudaRealTransform3D::executeX_DST3(double* d_data)
{
    ...
    cudaMemset(d_work_, 0, sizeof(double) * Ny_ * Nz_ * fftSize);
    kernel_dst3_3d_preOp_x<<<numBlocks_preOp, blockSize>>>(d_temp_, d_work_, Nx_, Ny_, Nz_);

    cufftDoubleComplex* d_freq = (cufftDoubleComplex*)d_x1_;
    cufftExecD2Z(plan_x_, d_work_, d_freq);

    kernel_dst3_3d_postOp_x<<<numBlocks_postOp, blockSize>>>(d_freq, d_data, Nx_, Ny_, Nz_);
}
