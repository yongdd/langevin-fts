...
template <typename T>
void CudaSolverPseudoRQM4<T>::cleanup_crysfft()
{
    use_crysfft_ = false;
    crysfft_mode_ = CudaCrysFFTMode::None;
    crysfft_identity_map_ = false;
            ...
            for(const auto& item: w_input)
            {
                ...
                ker_exp<<<N_BLOCKS, N_THREADS>>>
                    (this->d_exp_dw[ds_idx][monomer_type],
                     this->d_exp_dw[ds_idx][monomer_type], 1.0, -0.50*local_ds, M_use);
                // Compute d_exp_dw_half = exp(-w * local_ds * 0.25)
                ker_exp<<<N_BLOCKS, N_THREADS>>>
                    (this->d_exp_dw_half[ds_idx][monomer_type],
                     this->d_exp_dw_half[ds_idx][monomer_type], 1.0, -0.25*local_ds, M_use);
            }
        }
    ...
    try
    {
        ...
        if (use_crysfft)
        {
            ...
            else
            {
                ...
                double* d_full_step = reinterpret_cast<double*>(d_q_step_1_one[STREAM]);
                double* d_half_step = reinterpret_cast<double*>(d_q_step_2_one[STREAM]);
                ...
                if (crysfft_identity_map_)
                {
                    ...
                    crysfft_[STREAM]->diffusion(d_crysfft_in_[STREAM], d_full_step, streams[STREAM][0]);

                    // ===== Step 2: Two half steps =====
                    ker_multi<<<N_BLOCKS, N_THREADS, 0, streams[STREAM][0]>>>(
                        d_crysfft_in_[STREAM], d_q_in, _d_exp_dw_half, 1.0, M_phys);
                    gpu_error_check(cudaPeekAtLastError());
                    ...
                    ker_multi<<<N_BLOCKS, N_THREADS, 0, streams[STREAM][0]>>>(
                        d_crysfft_out_[STREAM], d_crysfft_out_[STREAM], _d_exp_dw, 1.0, M_phys);
                    gpu_error_check(cudaPeekAtLastError());

                    crysfft_[STREAM]->diffusion(d_crysfft_out_[STREAM], d_half_step, streams[STREAM][0]);

                    // ===== Richardson extrapolation with exp_dw factors =====
                    ker_lin_comb_exp<<<N_BLOCKS, N_THREADS, 0, streams[STREAM][0]>>>(
                        d_q_out,
                        d_half_step, _d_exp_dw_half, 4.0/3.0,
                        d_full_step, _d_exp_dw, -1.0/3.0,
                        d_q_mask, M_phys);
                    gpu_error_check(cudaPeekAtLastError());
                    return;
                }
                ...
                ker_reduce_to_basis<<<N_BLOCKS, N_THREADS, 0, streams[STREAM][0]>>>(
                    d_full_step, d_crysfft_out_[STREAM], d_crysfft_reduced_to_phys_, n_basis_);
                gpu_error_check(cudaPeekAtLastError());

                // ===== Step 2: Two half steps =====
                ker_expand_reduced_basis_multi<<<N_BLOCKS, N_THREADS, 0, streams[STREAM][0]>>>(
                    d_crysfft_in_[STREAM], d_q_in, _d_exp_dw_half, d_crysfft_phys_to_reduced_, M_phys);
                gpu_error_check(cudaPeekAtLastError());
                ...
                crysfft_[STREAM]->diffusion(d_crysfft_out_[STREAM], d_crysfft_in_[STREAM], streams[STREAM][0]);

                ker_multi_map<<<N_BLOCKS, N_THREADS, 0, streams[STREAM][0]>>>(
                    d_crysfft_in_[STREAM], _d_exp_dw_half, d_crysfft_phys_to_reduced_, M_phys);
                gpu_error_check(cudaPeekAtLastError());

                ker_reduce_to_basis<<<N_BLOCKS, N_THREADS, 0, streams[STREAM][0]>>>(
                    d_half_step, d_crysfft_in_[STREAM], d_crysfft_reduced_to_phys_, n_basis_);
                gpu_error_check(cudaPeekAtLastError());

                // ===== Richardson extrapolation on reduced basis =====
                ker_lin_comb<<<N_BLOCKS, N_THREADS, 0, streams[STREAM][0]>>>(
                    d_q_out, 4.0/3.0, d_half_step, -1.0/3.0, d_full_step, n_basis_);
                gpu_error_check(cudaPeekAtLastError());
    ...
    std::string monomer_type, [[maybe_unused]] bool is_half_bond_length)
{
    try{
        ...
        // fourier_basis arrays contain v⊗v dyad components of the deformation
        // vector v = 2π g⁻¹ m (units 1/L², NOT the Cartesian wavevector):
        // _d_fourier_basis_x = v₁², _d_fourier_basis_xy = v₁v₂, etc.
        // The accumulated sums V_ij = Σ kernel·v_i·v_j are converted to
        // lattice-parameter derivatives in compute_stress().
        ...
        // systems, so those slots are never consumed. This optimization
        ...
        if ( DIM == 2 )
        {
            ...
            // V₁₁
            ker_multi<<<N_BLOCKS, N_THREADS, 0, streams[STREAM][0]>>>(d_stress_sum[STREAM], d_q_multi[STREAM], _d_basis_xx, bond_length_sq, M_COMPLEX);
            gpu_error_check(cudaPeekAtLastError());
            if constexpr (std::is_same<T, double>::value)
                cub::DeviceReduce::Sum(d_temp_storage[STREAM], temp_storage_bytes[STREAM], d_stress_sum[STREAM], &d_segment_stress[0], M_COMPLEX, streams[STREAM][0]);
            else
            ...
            // V₂₂
            ker_multi<<<N_BLOCKS, N_THREADS, 0, streams[STREAM][0]>>>(d_stress_sum[STREAM], d_q_multi[STREAM], _d_basis_yy, bond_length_sq, M_COMPLEX);
            gpu_error_check(cudaPeekAtLastError());
            if constexpr (std::is_same<T, double>::value)
                cub::DeviceReduce::Sum(d_temp_storage[STREAM], temp_storage_bytes[STREAM], d_stress_sum[STREAM], &d_segment_stress[1], M_COMPLEX, streams[STREAM][0]);
            else
            ...
            // V₁₂: only compute for non-orthogonal boxes
            if (!is_orthogonal)
            {
                ker_multi<<<N_BLOCKS, N_THREADS, 0, streams[STREAM][0]>>>(d_stress_sum[STREAM], d_q_multi[STREAM], _d_basis_xy, bond_length_sq, M_COMPLEX);
                gpu_error_check(cudaPeekAtLastError());
                if constexpr (std::is_same<T, double>::value)
                    cub::DeviceReduce::Sum(d_temp_storage[STREAM], temp_storage_bytes[STREAM], d_stress_sum[STREAM], &d_segment_stress[2], M_COMPLEX, streams[STREAM][0]);
                else
