    ...
    try
    {
        ...
        for(const auto& d_block: d_phi_block)
        {
            ...
            single_partition_segment.push_back(std::make_tuple(
                p,
                d_propagator[dep_left][n_segment_left-1],    // q
                d_propagator[dep_right][0],                  // q_dagger
                monomer_type,       
       ...
       // Find propagators and bond length for each segment to prepare stress computation
        for(const auto& block: d_phi_block)
        {
            ...
            double **d_q_1 = d_propagator[dep_left];     // dependency v
            double **d_q_2 = d_propagator[dep_right];    // dependency u
            ...
            for(int n=0; n<=N; n++)
            {
                ...
                if (n == N_OFFSET)
                {
                    if (propagator_analyzer->get_computation_propagator_code(dep_left).deps.size() == 0) // if v is leaf node, skip
                    {
                        _block_stress_compuation_key.push_back(std::make_tuple(d_propagator_v, d_propagator_u, is_half_bond_length));
                        continue;
                    }
                    
                    d_propagator_v = d_propagator_junction[dep_left];
                    d_propagator_u = d_q_2[N-1];
                    is_half_bond_length = true;
                }
                // At u
                else if (n == 0 && dep_right.find('[') == std::string::npos){
                    if (propagator_analyzer->get_computation_propagator_code(dep_right).deps.size() == 0) // if u is leaf node, skip
                    {
                        _block_stress_compuation_key.push_back(std::make_tuple(d_propagator_v, d_propagator_u, is_half_bond_length));
                        continue;
                    }

                    d_propagator_v = d_q_1[N_OFFSET-1];
                    d_propagator_u = d_propagator_junction[dep_right];
                    is_half_bond_length = true;
                }
                ...
                {
                    d_propagator_v = d_q_1[N_OFFSET-n-1];
                    d_propagator_u = d_q_2[n-1];
    ...
    std::map<std::string, const double*> q_init)
{
    try
    {
        ...
        for(const auto& block: d_phi_block)
        {
            ...
            // Check keys
            #ifndef NDEBUG
            if (d_propagator.find(dep_left) == d_propagator.end())
                throw_with_line_number("Could not find dep_left key'" + dep_left + "'. ");
            if (d_propagator.find(dep_right) == d_propagator.end())
            ...
            calculate_phi_one_block(
                block.second,              // phi
                d_propagator[dep_left],    // dependency v
                d_propagator[dep_right],   // dependency u
                _d_exp_dw,                 // exp_dw
                n_segment_right,
                n_segment_left);
...
void CudaComputationDiscrete::calculate_phi_one_block(
    double *d_phi, double **d_q_1, double **d_q_2, double *d_exp_dw, const int N, const int N_OFFSET)
{
    try
    {
        gpu_error_check(cudaSetDevice(0));
        ...
        multi_real<<<N_BLOCKS, N_THREADS>>>(d_phi,d_q_1[N_OFFSET-1], d_q_2[0], 1.0, M);
        for(int n=1; n<N; n++)
        {
            add_multi_real<<<N_BLOCKS, N_THREADS>>>(d_phi, d_q_1[N_OFFSET-n-1], d_q_2[n], 1.0, M);
        }
...
void CudaComputationDiscrete::get_block_concentration(int p, double *phi)
{
    try
    {
        ...
        for(size_t b=0; b<blocks.size(); b++)
        {
            ...
            lin_comb<<<N_BLOCKS, N_THREADS>>>(d_phi, 0.0, d_phi, 1.0, d_phi_block[std::make_tuple(p, dep_left, dep_right)], M);
...
std::vector<double> CudaComputationDiscrete::compute_stress()
{
    ...
    try
    {
        ...
        for(size_t b=0; b<d_phi_block.size();b++)
        {
            ...
            double **d_q_1 = d_propagator[dep_left];      // Propagator q
            double **d_q_2 = d_propagator[dep_right];     // Propagator q^dagger
            ...
            if(_block_stress_compuation_key.size() != (unsigned int) (N+1))
            {
                throw_with_line_number("Mismatch of block_stress_computation_plan("
                    + std::to_string(p) + "," + dep_left + "," + dep_right + ") "
    ...
    for(const auto& block: d_phi_block)
    {
        ...
        for(int n=0;n<n_segment_right;n++)
        {
            double total_partition = cb->inner_product_inverse_weight_device(
                d_propagator[dep_left][(n_segment_left-n-1)],
                d_propagator[dep_right][n], _d_exp_dw)*n_repeated/cb->get_volume();
