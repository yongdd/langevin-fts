    ...
    std::map<std::string, const double*> q_init)
{
    try
    {
            ...
            for (size_t job = 0; job < parallel_job->size(); job++)
            {
                    ...
                    if (key[0] == '[')
                    {
                        ...
                        for (size_t d = 0; d < deps.size(); d++)
                        {
                            ...
                            ker_lin_comb<<<N_BLOCKS, N_THREADS, 0, streams[STREAM][0]>>>(
                                ...
                                sub_n_repeated, _d_sub_half[2 * sub_n_segment], M);
                        }
                        ...
                        for (size_t d = 0; d < deps.size(); d++)
                        {
                            ...
                            for (int r = 0; r < sub_n_repeated; r++)
                            {
                                ...
                                ker_multi<<<N_BLOCKS, N_THREADS, 0, streams[STREAM][0]>>>(
                                    ...
                                    _d_sub_half[2 * sub_n_segment], 1.0, M);
                            }
                        }
                        gpu_error_check(cudaPeekAtLastError());
                    }
                ...
                for (int n = n_segment_from; n < n_segment_to; n++)
                {
                    ...
                    solver->advance_half_step(
                        STREAM, _d_prop_half[half_idx], _d_prop_half[half_idx + 1], monomer_type, d_q_mask);
                    solver->advance_half_step(
                        STREAM, _d_prop_half[half_idx + 1], _d_prop_half[half_idx + 2], monomer_type, d_q_mask);
        ...
        for (const auto& item : propagator_computation_optimizer->get_computation_propagators())
        {
            ...
            {
                // q_rich[n] = (4.0 * q_half[2n] - q_full[n]) / 3.0
                ker_lin_comb<<<N_BLOCKS, N_THREADS>>>(
                    _d_prop_rich[n], 4.0/3.0, _d_prop_half[2 * n],
    ...
    double* q_out, int polymer, int v, int u, int n)
{
    try
    {
        ...
        gpu_error_check(cudaMemcpy(q_out, d_propagator_half[dep][n], sizeof(double)*M, cudaMemcpyDeviceToHost));
    }
    catch (std::exception& exc)
    {
        throw_without_line_number(exc.what());
    ...
    for (const auto& block : d_phi_block)
    {
        ...
        for (int n = 0; n <= 2 * n_segment_right; n++)
        {
            double total_partition = dynamic_cast<CudaComputationBox<double>*>(cb)->inner_product_device(
                d_propagator_half[key_left][2 * n_segment_left - n],
                d_propagator_half[key_right][n]) * (n_repeated / cb->get_volume());
