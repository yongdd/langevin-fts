...
void CpuPseudoContinuous::get_total_concentration(std::string monomer_type, double *phi)
{
    try
    {
        const int M = cb->get_n_grid();
...
void CpuPseudoContinuous::get_total_concentration(int p, std::string monomer_type, double *phi)
{
    try
    {
        const int M = cb->get_n_grid();
...
void CpuPseudoContinuous::get_block_concentration(int p, double *phi)
{
    try
    {
        const int M = cb->get_n_grid();
...
double CpuPseudoContinuous::get_solvent_partition(int s)
{
    try
    {
...
void CpuPseudoContinuous::get_solvent_concentration(int s, double *phi)
{
    try
    {
        const int M = cb->get_n_grid();
...
bool CpuPseudoContinuous::check_total_partition()
{
    const int M = cb->get_n_grid();
