 ...
 *
 * This header provides a compile-time selection between MKL and FFTW FFT
 * backends. Include this header instead of MklFFT.h or FftwFFT.h directly
 * in solver code to enable backend switching via preprocessor defines.
 *
 * **Backend Selection:**
 *
 * - USE_CPU_MKL: Use Intel MKL's DFTI for FFT
...
/**
 * @brief Type alias for MKL-based FFT.
 * @tparam T Numeric type (double or std::complex<double>)
 * @tparam DIM Dimensionality (1, 2, or 3)
 */
template <typename T, int DIM>
using CpuFFT = MklFFT<T, DIM>;
...
#else
#error "No CPU FFT backend selected. Define USE_CPU_MKL or USE_CPU_FFTW."
#endif
