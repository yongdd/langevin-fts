...
void CpuSolverDiscrete::update_bond_function()
{
    try
    {
...
double CpuSolverDiscrete::get_total_partition(int polymer)
{
    try
    {
...
void CpuSolverDiscrete::get_total_concentration(std::string monomer_type, double *phi)
{
    try
    {
        const int M = cb->get_n_grid();
...
void CpuSolverDiscrete::get_total_concentration(int p, std::string monomer_type, double *phi)
{
    try
    {
        const int M = cb->get_n_grid();
...
void CpuSolverDiscrete::get_block_concentration(int p, double *phi)
{
    try
    {
        const int M = cb->get_n_grid();
...
double CpuSolverDiscrete::get_solvent_partition(int s)
{
    try
    {
...
void CpuSolverDiscrete::get_solvent_concentration(int s, double *phi_out)
{
    try
    {
        const int M = cb->get_n_grid();
...
std::vector<double> CpuSolverDiscrete::compute_stress()
{
...
void CpuSolverDiscrete::get_chain_propagator(double *q_out, int polymer, int v, int u, int n)
{ 
    // This method should be invoked after invoking compute_statistics()
...
bool CpuSolverDiscrete::check_total_partition()
{
    const int M = cb->get_n_grid();
