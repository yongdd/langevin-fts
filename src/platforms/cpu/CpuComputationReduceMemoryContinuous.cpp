 ...
 *                                       - For pseudospectral: "rqm4" or "rk2"
 *                                       - For realspace: "cn-adi2"
 */
template <typename T>
    ...
    try
    {
        ...
        if(this->method == "pseudospectral")
        {
            if (numerical_method == "" || numerical_method == "rqm4")
                ...
                throw_with_line_number("Unknown pseudo-spectral this->method: '" + numerical_method + "'. Use 'rqm4' or 'rk2'.");
        }
        else if(this->method == "realspace")
        {
            if constexpr (std::is_same<T, double>::value)
            {
                ...
                {
                    throw_with_line_number("Unknown realspace this->method: '" + numerical_method + "'. Use 'cn-adi2'.");
                }
        ...
        for(const auto& item: this->propagator_computation_optimizer->get_computation_propagators())
        {
            ...
            if(this->propagator_at_check_point.find(std::make_tuple(key, 0)) == this->propagator_at_check_point.end())
                this->propagator_at_check_point[std::make_tuple(key, 0)] = new T[N];
        ...
        for(const auto& block: this->phi_block)
        {
            ...
            {
                this->propagator_at_check_point[std::make_tuple(key_right, 0)] = new T[N];
                #ifndef NDEBUG
...
void CpuComputationReduceMemoryContinuous<T>::compute_concentrations()
{
    try
    {
        ...
        for(size_t b=0; b<this->phi_block.size();b++)
        {
            ...
            // Result is stored in reduced basis if using space group
...
template <typename T>
std::vector<T*> CpuComputationReduceMemoryContinuous<T>::recalculate_propagator(std::string key, const int N_START, const int N_RIGHT, std::string monomer_type)
{
    ...
    try
    {
        ...
        for(int n = prev_checkpoint + 1; n <= N_RIGHT; n++)
        {
            ...
            else
            {
                ...
                {
                    throw_with_line_number("Workspace overflow in recalculate_propagator at ws_idx=" +
