    ...
    try
    {
        ...
        for (const auto& block : phi_block)
        {
            ...
            partition_segment_info.push_back(std::make_tuple(
                p,
                propagator_full[key_left][n_segment_left],   // q_full at end
                propagator_full[key_right][0],                // q†_full at start
                propagator_half[key_left][n_segment_left],   // q_half at end
                propagator_half[key_right][0],                // q†_half at start
                n_aggregated
            ));
            current_p++;
        }
    ...
    try
    {
        ...
        for (auto parallel_job = branch_schedule.begin(); parallel_job != branch_schedule.end(); parallel_job++)
        {
            ...
            for (size_t job = 0; job < parallel_job->size(); job++)
            {
                    ...
                    if (key[0] == '[')
                    {
                        ...
                        for (size_t d = 0; d < deps.size(); d++)
                        {
                            ...
                            for (int i = 0; i < M; i++)
                            {
                                ...
                                _prop_half[0][i] += _sub_half[sub_n_segment][i] * static_cast<double>(sub_n_repeated);
                            }
                        }
                    }
                    else if (key[0] == '(')
                    {
                        ...
                        for (size_t d = 0; d < deps.size(); d++)
                        {
                            ...
                            for (int i = 0; i < M; i++)
                            {
                                ...
                                _prop_half[0][i] *= pow(_sub_half[sub_n_segment][i], sub_n_repeated);
                            }
                        }
                    }
                ...
                // Allocate temporary buffer for intermediate half-step
                double* q_half_temp = new double[M];
                ...
                for (int n = n_segment_from; n < n_segment_to; n++)
                {
                    ...
                    // Use temporary buffer for intermediate value (odd position)
                    // _prop_half[n] -> q_half_temp -> _prop_half[n+1]
                    solver->advance_half_step(
                        _prop_half[n], q_half_temp, monomer_type, q_mask);
                    solver->advance_half_step(
                        q_half_temp, _prop_half[n + 1], monomer_type, q_mask);
        ...
        // Compute Richardson-extrapolated propagators: q_rich[n] = (4·q_half[n] - q_full[n]) / 3
        // Note: q_half[n] stores the half-step chain value at the same contour position as q_full[n]
        for (const auto& item : propagator_computation_optimizer->get_computation_propagators())
        {
            std::string key = item.first;
            ...
            for (int n = 0; n <= n_segment; n++)
            {
                for (int i = 0; i < M; i++)
                {
                    _prop_rich[n][i] = (4.0 * _prop_half[n][i] - _prop_full[n][i]) / 3.0;
                }
            }
        }
...
void CpuComputationGlobalRichardson::compute_concentrations()
{
    try
    {
        ...
        for (size_t b = 0; b < phi_block.size(); b++)
        {
            ...
            double Q_full = cb->inner_product(
                propagator_full[key_left][n_segment_left],
                propagator_full[key_right][0]) / cb->get_volume();
            double Q_half = cb->inner_product(
                propagator_half[key_left][n_segment_left],
                propagator_half[key_right][0]) / cb->get_volume();
            ...
            calculate_phi_one_block(
                block->second,
                propagator_full[key_left], propagator_full[key_right],
                propagator_half[key_left], propagator_half[key_right],
                n_segment_left,
                n_segment_right,
                Q_full,
                Q_half,
    ...
    const double norm)
{
    try
    {
        ...
        std::vector<double> phi_full(M, 0.0);
        for (int n = 0; n <= N_RIGHT; n++)
        {
            ...
            for (int i = 0; i < M; i++)
            {
                phi_full[i] += simpson_coeff[n] * q_1_full[idx_1][i] * q_2_full[idx_2][i];
            }
        }
        for (int i = 0; i < M; i++)
            phi_full[i] *= norm_full;
        ...
        for (int n = 0; n <= N_RIGHT; n++)
        {
            ...
            for (int i = 0; i < M; i++)
            {
                phi[i] += simpson_coeff[n] * q_1_half[idx_1][i] * q_2_half[idx_2][i];
            }
        }
        for (int i = 0; i < M; i++)
            phi[i] *= norm_half;
        ...
        for (int i = 0; i < M; i++)
        {
            phi[i] = (4.0 * phi[i] - phi_full[i]) / 3.0;
        }
    }
    catch (std::exception& exc)
    {
    ...
    double* q_out, int polymer, int v, int u, int n)
{
    try
    {
        ...
        double** _prop_rich = propagator_richardson[dep];
        for (int i = 0; i < M; i++)
            q_out[i] = _prop_rich[n][i];
    }
    catch (std::exception& exc)
    {
        throw_without_line_number(exc.what());
...
bool CpuComputationGlobalRichardson::check_total_partition()
{
    ...
    for (const auto& block : phi_block)
    {
        ...
        // Check at full-step points using Richardson-extrapolated propagators
        for (int n = 0; n <= n_segment_right; n++)
        {
            double total_partition = cb->inner_product(
                propagator_richardson[key_left][n_segment_left - n],
                propagator_richardson[key_right][n]) * (n_repeated / cb->get_volume());
    ...
    for (size_t p = 0; p < total_partitions.size(); p++)
    {
        ...
        if (diff > 1e-7)  // Tolerance for Richardson propagators
            return false;
    }
