/**
 * @file CpuComputationReduceMemoryBase.cpp
 * @brief Common base class implementation for memory-efficient CPU propagator computation.
 *
 * Implements shared functionality between CpuComputationReduceMemoryContinuous
 * and CpuComputationReduceMemoryDiscrete, including concentration queries and
 * partition function accessors.
 *
 * @see CpuComputationReduceMemoryBase.h for class documentation
 */
...
template <typename T>
CpuComputationReduceMemoryBase<T>::CpuComputationReduceMemoryBase(
    ComputationBox<T>* cb,
    Molecules *molecules,
...
template <typename T>
void CpuComputationReduceMemoryBase<T>::update_laplacian_operator()
{
    try
    {
...
template <typename T>
void CpuComputationReduceMemoryBase<T>::compute_statistics(
...
template <typename T>
void CpuComputationReduceMemoryBase<T>::get_total_concentration(std::string monomer_type, T *phi)
{
    try
    {
...
template <typename T>
void CpuComputationReduceMemoryBase<T>::get_total_concentration(int p, std::string monomer_type, T *phi)
{
    try
    {
        const int M = this->cb->get_total_grid();
...
template <typename T>
void CpuComputationReduceMemoryBase<T>::get_total_concentration_gce(double fugacity, int p, std::string monomer_type, T *phi)
{
    try
    {
        const int M = this->cb->get_total_grid();
...
template <typename T>
void CpuComputationReduceMemoryBase<T>::get_block_concentration(int p, T *phi)
{
    try
    {
        const int M = this->cb->get_total_grid();
...
template <typename T>
T CpuComputationReduceMemoryBase<T>::get_solvent_partition(int s)
{
    try
    {
...
template <typename T>
void CpuComputationReduceMemoryBase<T>::get_solvent_concentration(int s, T *phi)
{
    try
    {
        const int M = this->cb->get_total_grid();
...
INSTANTIATE_CLASS(CpuComputationReduceMemoryBase);
