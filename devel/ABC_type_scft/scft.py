...
class SCFT:
    def __init__(self, params):
        ...
        distinct_polymers = []
        assert(len(params['segment_lengths']) <= 3), \
            "Currently, only ABC-type polymers are supported."
        assert( len(set(["A","B","C"]).intersection(set(params['segment_lengths'].keys()))) == len(params['segment_lengths'].keys()) ), \
            "Use letters 'A', 'B' and 'C' for monomer types."
        ...
        for polymer_counter, polymer in enumerate(params["distinct_polymers"]):
            ...
            for block in polymer["blocks"]:
                ...
                if "random" in block["type"]:
                    if block["type"] in monomer_fraction:
                        for random_monomer_type in block["fraction"]:
                            monomer_fraction[random_monomer_type] += block["length"]*block["fraction"][random_monomer_type]
                    else:
                        for random_monomer_type in block["fraction"]:
                            monomer_fraction[random_monomer_type] = block["length"]*block["fraction"][random_monomer_type]
                else:
                    if block["type"] in monomer_fraction:
                        monomer_fraction[block["type"]] += block["length"]
                    else:
                        monomer_fraction[block["type"]] = block["length"]
                block_count += 1
            ...
            for monomer_type in monomer_fraction:
                monomer_fraction[monomer_type] /= monomer_fraction[monomer_type]/alpha
                statistical_segment_length += params["segment_lengths"][monomer_type]**2 * monomer_fraction[monomer_type]
            ...
            if "random" in set(bt.lower() for bt in block_monomer_type_list):
                ...
                params["segment_lengths"].update({f"random{polymer_counter}":statistical_segment_length})
                self.random_copolymer_exist = True
                self.random_fraction[polymer_counter] = monomer_fraction
       ...
       # The maximum iteration steps
        ...
        print("chi_n: ")
        for pair in params["chi_n"]:
            print("\t%s, %s: %f" % (sorted(list(pair))[0], sorted(list(pair))[1], params["chi_n"][pair]))
        print("Conformational asymmetry (epsilon): ")
        if "A" in params["segment_lengths"] and "B" in params["segment_lengths"]:
            print("\tA/B: %f" % (params["segment_lengths"]["A"]/params["segment_lengths"]["B"]))
        if "A" in params["segment_lengths"] and "C" in params["segment_lengths"]:
            print("\tA/C: %f" % (params["segment_lengths"]["A"]/params["segment_lengths"]["C"]))
        if "B" in params["segment_lengths"] and "C" in params["segment_lengths"]:
            print("\tB/C: %f" % (params["segment_lengths"]["B"]/params["segment_lengths"]["C"]))
