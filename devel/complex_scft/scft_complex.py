...
class SCFT:
    def __init__(self, params): #, phi_target=None, mask=None):
        ...
        if "use_checkpointing" in params and platform == "cuda":
            factory = PlatformSelector.create_factory(platform, params["use_checkpointing"], "complex")
        else:
    ...
    def run(self, initial_fields, q_init=None):
        ...
        for scft_iter in range(1, self.max_iter+1):
            ...
            if (self.box_is_altering):
                ...
                new_lx = np.real(np.array(am_new[-self.cb.get_dim():]))
        ...
        for p in range(self.molecules.get_n_polymer_types()):
            ...
            print("\tβF/n_%d : " % (p+1), energy_total_per_chain)
