...
class SCFT:
    def __init__(self, params, phi_target=None, mask=None):
        ...
        cb = factory.create_computation_box(params["nx"], params["lx"], bc=params["boundary_conditions"], mask=mask)
        ...
        # (C++ class) Solver using real space method
    ...
    def run(self, initial_fields, q_init=None):
        ...
        alpha = self.molecules.get_polymer(0).get_alpha()
        
        if self.cb.get_dim()==3:
            polymer_volume = alpha*self.params["lx"][1]*self.params["lx"][2]
            box_volume = self.params["lx"][0]*self.params["lx"][1]*self.params["lx"][2]
        elif self.cb.get_dim()==2:
            polymer_volume = alpha*self.params["lx"][1]
            box_volume = self.params["lx"][0]*self.params["lx"][1]
        elif self.cb.get_dim()==1:
            polymer_volume = alpha
            box_volume = self.params["lx"][0]
            
        available_volume = self.cb.get_volume()
        
        volume_scaling = alpha/self.params["lx"][0]
        # print(available_volume, box_volume)
        # print(volume_scaling_partition)
        # print(np.log(volume_scaling_partition))
        ...
        for scft_iter in range(1, self.max_iter+1):
            ...
            # Scaling phi
            for monomer_type in self.monomer_types:
                phi[monomer_type] *= volume_scaling
            ...
            # energy_total = -np.mean(self.phi_target*mask*w_exchange[S-1])
            # for i in range(S-1):
            #     energy_total -= 0.5/self.exchange_eigenvalues[i]*np.dot(w_exchange[i]*mask,w_exchange[i])/self.cb.get_n_grid()
            # for i in range(S-1):
            #     for j in range(S-1):
            #         energy_total += 1.0/self.exchange_eigenvalues[i]*self.matrix_o[j,i]*self.vector_s[j]*np.mean(w_exchange[i]*mask)
            ...
            energy_total = - self.cb.integral(w[0]*phi["A"])/polymer_volume/2
            for p in range(self.molecules.get_n_polymer_types()):
                energy_total -= self.molecules.get_polymer(p).get_volume_fraction()/ \
                                self.molecules.get_polymer(p).get_alpha() * \
                                np.log(self.solver.get_total_partition(p))   
            ...
            # for i in range(S):
            #     w_diff[i] *= mask
            #     w_diff[i] -= np.mean(w_diff[i]*mask)
            w_diff[0,:] = (lamb*phi["A"] - w[0,:])*self.mask
